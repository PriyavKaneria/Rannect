defmodule RannectWeb.Components.OnlineUsers do
  ...
  def render(assigns) do
    ~H"""
    <ul role="list" class="flex flex-wrap w-full duration-200 space-x-5">
      <%= for {user_id, user} <- @users do %>
        <%= if @type=="rannections" and user.chatting do %>
          <li class="col-span-1 flex flex-col text-center bg-white rounded-lg shadow divide-y divide-gray-200 hover:shadow-xl duration-600 w-1/3">
            <div class="chat-modal show flex flex-col shadow-lg">
              <!-- admin profile -->
              <div class="flex justify-between items-center text-white p-2 bg-indigo-500 border shadow-lg mr-5 w-full">
              ...
              <!-- chats -->
              <div class="flex flex-col bg-gray-200 px-2 chat-services expand overflow-auto h-56">
                <div class="chat bg-white text-gray-700 p-2 self-start my-2 rounded-md shadow mr-3">
                  apa ada yang bisa saya bantu ?
                </div>
                <div class="message bg-indigo-500 text-white p-2 self-end my-2 rounded-md shadow ml-3">
              ...
              <div class="relative bg-white">
                <input type="text" name="message" placeholder="ketik pesan anda"
                      class="pl-4 pr-16 py-2 border border-indigo-500 focus:outline-none w-full">
                  <button class="absolute right-0 bottom-0 text-indigo-600 bg-white  hover:text-indigo-500 m-1 px-3 py-1 w-auto transistion-color duration-100 focus:outline-none">Send</button>
              </div>
            </div>
          </li>
        <% else %>
            ...
            <li class="col-span-1 flex flex-col text-center bg-white rounded-lg shadow divide-y divide-gray-200 hover:shadow-xl duration-600 w-1/3">
              <div class="flex-1 flex flex-col p-5">
                ...
                <dl class="mt-1 flex-grow flex flex-col justify-between">
                  ...
                  <dd class="mt-3">
                    <span class="px-2 py-1 text-indigo-800 text-xs font-medium bg-indigo-100 rounded-full capitalize"><%= user[:gender] %></span>
                  </dd>
                </dl>
              </div>
              <div>
                  <%= if @type == "users" do %>
                    <%= if Enum.find_value(@sent, false, fn x -> x[:id] == String.to_integer(user_id) end)==true do %>
                      <div class="flex-1 flex px-1 text-indigo-200-contrast hover:text-indigo-300-contrast bg-indigo-200 hover:bg-indigo-300 rounded-b rounded-sm">
                  ...
                  <%= if @type == "rannections" do %>
                    <div class="flex-1 flex px-1 text-indigo-200-contrast hover:text-indigo-300-contrast bg-indigo-200 hover:bg-indigo-300 rounded-b rounded-sm" phx-click="chat" phx-value-userid={user_id}>
                      <div class="relative px-1 flex-1 inline-flex items-center justify-center py-4 text-sm font-medium border border-transparent rounded-br-lg">
                        <!-- Heroicon name: solid/phone -->
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
